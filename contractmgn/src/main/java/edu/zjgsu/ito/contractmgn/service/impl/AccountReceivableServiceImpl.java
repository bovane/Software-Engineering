...
public class AccountReceivableServiceImpl implements AccountReceivableService {
    ...
    @Override
    public Message addOperationAccountReceivable(OperationNode operationNode) throws ParseException {
...
//                accountReceivable.setStartDate(startTime);
    ...
    @Override
    public Message addCommonAccountReceivable(AccountReceivable accountReceivable) {
    ...
    public Boolean isLater(Integer accountReId, String nodeName, Integer contractId, Date nodeDate) {
        ...
        if (integerList.size() > 0) {
            ...
            if (accountReceivableDAO.selectByExample(are).size() > 0) {
                ...
                if (accountRe.getEndDate().before(nodeDate) && accountRe.getId() != accountReId) {//与后面节点最早收款日期比较
                    return true;
                }
            }
        }
