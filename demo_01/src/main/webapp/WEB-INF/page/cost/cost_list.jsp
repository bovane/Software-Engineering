...
//$()可以是$(expresion)，即css选择器、Xpath或html元素，也就是通
                    过上述表达式来匹配目标元素
//是Juery的自定义函数的写法
...
//this.checked表示后面每一个checkbox的选择状态与第一个checkbox的选
                择状态一样
            });
        }
    </script>
...
5px;">
        <div class="form-group">
            <label>费用名称 </label> <input type="text" class="form-
control"
                                        name="costName" placeholder=" 请输入费用名称 "/>
        </div>
        <input type="submit" value=" 查询费用 " class="btn btn-
success"/>
        <a href="cost/loadadd.do" class="btn btn-info">添加费用</a>
        <input type="button" id="update" value=" 修改费用 " class="btn
btn-primary"/>
        <input type="button" id="delete" value=" 删除费用 " class="btn
btn-danger"/>
    </div>
</form>
